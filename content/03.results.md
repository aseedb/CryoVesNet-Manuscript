...
![**Dice development during post-processing**. Dice development at different post-processing steps of initially predicted mask (different colors correspond to different tomograms): A) synaptosomal train set B) synaptosomal test set c)  neuronal generalization test set](images/multi_panel_plot_vertical.png){#fig:dice-improv width="15cm"}
