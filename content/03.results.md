...
3) Testing dataset for generizibility: 12 mouse primary neuronal culture tomograms). 
...
![**Vesicle radius and position refinment through radial profile and cross-correlation**. A) Initial segmentation of a vesicle. B) Radial Profile. Blue range is from membrane center to outer white halo center. This is defined as the search range for the optimal radius. C) second derivative of radial profile, used to define the exact edge of the membrane. D)  ](images/radial_avg_115-139.svg){#fig:radial_profile width="15cm"}
...
The Dice coefficient was used to quantitatively assess the accuracy of the annotation within the different tomograms for each step of our pipeline {@fig:dice-improv;@tbl:train-tomograms;@tbl:test-tomograms;@tbl:neuron-tomograms}.

(images/train-dice.png){#fig:dice-improv width="15cm"}
(images/test-dice.png){#fig:dice-improv width="15cm"}

![**Dice developement during post-processing** Dice developement at different post processing steps of initial predicted mask (different colors correspond to different tomograms): A) synaptosomal training datasets B) synaptosomal test datasets c) neuron test datasets]((images/train-dice.png)(images/test-dice.png)images/neuro-dice.png){#fig:dice-improv width="15cm"}(images/train-dice.png){#fig:dice-improv width="15cm"}
