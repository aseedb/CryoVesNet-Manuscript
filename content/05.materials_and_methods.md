...
![**Multidimensional Outlier Detection**. The scatter plot (left panel) represents vesicle features in the space defined by membrane density, radius, and thickness, with points colored according to the p-value of their Mahalanobis distance, identifying potential outliers. Central panels: outliers are highlighted: outlier 1 (red, top row) is segmented as a non-vesicular membrane-bound structure and is removed after this step. Outlier 2 (green, middle row), though correctly segmented, is flagged for its abnormally high radius and subsequently removed. Outlier 3 (blue, bottom row) is initially detected but is  misplaced. Right panels: outliers have been either removed (top and middle row) or fixed by refinement, In addition the right panels show the final vesicle segmentation boundaries. Bars, 20 nm.](images/new_outlier_figure.png){#fig:outlier width="15cm"}
